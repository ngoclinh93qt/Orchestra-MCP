...
describe("run-openai-tunnel script", () => {
  it("starts the bridge and tunnel together without putting a runtime key in argv", async () => {
    ...
    expect(script).toContain("process.env.AGENT_BRIDGE_AUTH_MODE ?? \"oauth\"");
    expect(script).toContain('AGENT_BRIDGE_AUTH_MODE="$AUTH_MODE" node --env-file=.env dist/main.js');
