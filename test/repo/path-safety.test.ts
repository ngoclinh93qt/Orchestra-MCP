...
describe("resolveAllowedFile", () => {
  it("returns the canonical path for a file inside an allowed root", async () => {
    ...
    await expect(resolveAllowedFile(file, filesPolicyFor(root))).resolves.toBe(canonicalFile);
  });
  ...
  it("rejects a file outside every allowed root", async () => {
    ...
    await expect(resolveAllowedFile(outside, filesPolicyFor(root))).rejects.toBeInstanceOf(PathNotAllowedError);
  });
  ...
  it("rejects a symlink escape", async () => {
    ...
    await expect(resolveAllowedFile(link, filesPolicyFor(root))).rejects.toBeInstanceOf(PathNotAllowedError);
  });
  ...
  it("refuses an .env file even when it is inside an allowed root", async () => {
    ...
    await expect(resolveAllowedFile(envFile, filesPolicyFor(root))).rejects.toBeInstanceOf(PathNotAllowedError);
  });
  ...
  it("rejects a directory passed where a file is expected", async () => {
    ...
    await expect(resolveAllowedFile(dir, filesPolicyFor(root))).rejects.toBeInstanceOf(PathNotAllowedError);
  });
  ...
  it("refuses a file directly requested inside .git, even though it is in an allowed root", async () => {
    ...
    await expect(resolveAllowedFile(gitConfig, filesPolicyFor(root))).rejects.toBeInstanceOf(PathNotAllowedError);
  });
  ...
  it("refuses a file nested deep inside node_modules", async () => {
    ...
    await expect(resolveAllowedFile(file, filesPolicyFor(root))).rejects.toBeInstanceOf(PathNotAllowedError);
  });
  ...
  it("still allows an ordinary nested file whose segments are not ignored", async () => {
    ...
    await writeFile(file, "export {}");
    await expect(resolveAllowedFile(file, filesPolicyFor(root))).resolves.toBe(await realpath(file));
  });
});
...
describe("resolveAllowedDirectoryStrict", () => {
  it("returns the canonical path for an ordinary directory inside an allowed root", async () => {
    ...
    await mkdir(dir, { recursive: true });
    await expect(resolveAllowedDirectoryStrict(dir, filesPolicyFor(root))).resolves.toBe(await realpath(dir));
  });
  ...
  it("refuses a directory that is itself an ignored name", async () => {
    ...
    await expect(resolveAllowedDirectoryStrict(join(root, "node_modules"), filesPolicyFor(root))).rejects.toBeInstanceOf(
      PathNotAllowedError,
    );
  });
  ...
  it("refuses a directory nested under an ignored segment", async () => {
    ...
    await expect(resolveAllowedDirectoryStrict(join(root, ".git", "refs"), filesPolicyFor(root))).rejects.toBeInstanceOf(
      PathNotAllowedError,
    );
  });
  ...
  it("converts a nonexistent path's raw ENOENT into a typed PathNotAllowedError", async () => {
    ...
    const root = join(base, "root");
    await mkdir(root, { recursive: true });
    await expect(resolveAllowedDirectoryStrict(join(root, "nope"), filesPolicyFor(root))).rejects.toBeInstanceOf(
      PathNotAllowedError,
    );
  });
  ...
  it("still refuses a directory outside every allowed root", async () => {
    ...
    await expect(resolveAllowedDirectoryStrict(outside, filesPolicyFor(root))).rejects.toBeInstanceOf(PathNotAllowedError);
  });
});
...
describe("isPathPermitted", () => {
  it("returns true for a path inside a root and false outside it", async () => {
    ...
    await mkdir(outside, { recursive: true });
    await expect(isPathPermitted(inside, filesPolicyFor(root))).resolves.toBe(true);
    await expect(isPathPermitted(outside, filesPolicyFor(root))).resolves.toBe(false);
  });

  it("returns false for a path that does not exist", async () => {
    const base = await mkdtemp(join(tmpdir(), "repo-safety-"));
    await expect(isPathPermitted(join(base, "does-not-exist"), filesPolicyFor(base))).resolves.toBe(false);
  });
});
